...
class TemporalModelBase(nn.Module):
    ...
    def forward(self, x):
        ...
        sz = x.shape[:3]
        x = x.view(x.shape[0], x.shape[1], -1)  # x (B,T,C) C = J*2
        x = x.permute(0, 2, 1)  # x (B,C,T) C = J*2 T = W^B
        
        x = self._forward_blocks(x)
        
        x = x.permute(0, 2, 1)  # x (B,T,C) C = J*3 T = 1
        x = x.view(sz[0], -1, self.num_joints_out, 3)  # x (B,T,J,3)
